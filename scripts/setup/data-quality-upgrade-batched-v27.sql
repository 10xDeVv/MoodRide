-- Batched national data-quality upgrade (single-pass, resumable).
--
-- Usage:
--   psql -d moodride -v ON_ERROR_STOP=1 -v chunk_size=500 -f scripts/setup/data-quality-upgrade-batched.sql
--
-- Notes:
-- - Targets only tiles not already at scoring_version '2.7-raster-data-quality-upgrade-national-batched'.
...
\echo Building national target batches for 2.7...
...
WHERE COALESCE(sst.scoring_version, '') <> '2.7-raster-data-quality-upgrade-national-batched';
...
WHERE scoring_version = '2.7-raster-data-quality-upgrade-national-batched';
