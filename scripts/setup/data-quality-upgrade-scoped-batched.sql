-- Batched scoped data-quality upgrade over the area covered by elevation_raster.
--
-- Usage (psql):
--   psql -d moodride -v ON_ERROR_STOP=1 -v chunk_size=500 -f scripts/setup/data-quality-upgrade-scoped-batched.sql
--
-- Notes:
-- - Processes only H3 tiles that intersect elevation_raster.
-- - Executes each batch as an independent statement via \gexec so work commits incrementally.
-- - Safe to re-run; updates are idempotent for the target tile set.
...
\echo Building target tile list over DEM footprint...
DROP TABLE IF EXISTS public.dq_target_h3_batches;
DROP TABLE IF EXISTS public.dq_elevation_footprint;

CREATE UNLOGGED TABLE public.dq_elevation_footprint AS
SELECT
    ST_UnaryUnion(ST_Collect(ST_ConvexHull(rast)))::geometry AS geom
FROM elevation_raster;

CREATE INDEX IF NOT EXISTS dq_elevation_footprint_geom_idx
    ON public.dq_elevation_footprint
    USING GIST (geom);
ANALYZE public.dq_elevation_footprint;

CREATE UNLOGGED TABLE public.dq_target_h3_batches AS
SELECT
    sst.h3_index,
    ROW_NUMBER() OVER (ORDER BY sst.h3_index) AS rn
FROM scenic_score_tiles sst
JOIN public.dq_elevation_footprint ef
  ON ST_Intersects(
        CASE
            WHEN ST_SRID(sst.geometry) = ST_SRID(ef.geom) THEN sst.geometry
            ELSE ST_Transform(sst.geometry, ST_SRID(ef.geom))
        END,
        ef.geom
     )
WHERE COALESCE(sst.scoring_version, '') <> '2.4-raster-data-quality-upgrade-scoped-batched';

CREATE UNIQUE INDEX IF NOT EXISTS dq_target_h3_batches_h3_idx
    ON public.dq_target_h3_batches (h3_index);
CREATE INDEX IF NOT EXISTS dq_target_h3_batches_rn_idx
    ON public.dq_target_h3_batches (rn);
ANALYZE public.dq_target_h3_batches;

SELECT COUNT(*) AS target_tiles, MIN(rn) AS min_rn, MAX(rn) AS max_rn
FROM public.dq_target_h3_batches;
...
WITH bounds AS (
    SELECT
        ...
        LEAST(
            ...
            (SELECT COALESCE(MAX(rn), 0) FROM public.dq_target_h3_batches)
        ) AS end_rn
...
BEGIN
    ...
    SET scenic_score = LEAST(
        ...
        scoring_version = '2.4-raster-data-quality-upgrade-scoped-batched'
    WHERE EXISTS (
        SELECT 1
        FROM batch_h3 b
...
\echo Final stats (target subset + global)
SELECT
    COUNT(*) AS target_tiles,
    COUNT(*) FILTER (WHERE sst.green_score > 0.0) AS green_non_zero_tiles,
    COUNT(*) FILTER (WHERE sst.solitude_score > 0.0) AS solitude_non_zero_tiles,
    COUNT(*) FILTER (WHERE sst.elevation_score > 0.0) AS elevation_non_zero_tiles,
    ROUND(AVG(sst.green_score)::numeric, 6) AS avg_green_score,
    ROUND(STDDEV_POP(sst.green_score)::numeric, 6) AS stddev_green_score,
    ROUND(AVG(sst.elevation_score)::numeric, 6) AS avg_elevation_score,
    ROUND(STDDEV_POP(sst.elevation_score)::numeric, 6) AS stddev_elevation_score,
    ROUND(AVG(sst.scenic_score)::numeric, 6) AS avg_scenic_score,
    ROUND(STDDEV_POP(sst.scenic_score)::numeric, 6) AS stddev_scenic_score
FROM scenic_score_tiles sst
WHERE EXISTS (
    SELECT 1
    FROM public.dq_target_h3_batches t
