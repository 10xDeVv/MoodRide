-- DataQualityUpgrade execution SQL
-- Source plan: docs/DataQualityUpgrade.md
--
-- Expected order:
-- 1) Load land cover raster into public.landcover_raster
-- 2) Load elevation raster into public.elevation_raster
-- 3) Run this script
...
-- Class mapping table is intentionally editable. Tune values once you validate
-- the exact class legend for your selected Canada Land Cover product version.
...
-- Default seed mapping supports both common NLCD classes and common Canada
-- land-cover class IDs. Adjust these values to your exact dataset legend.
INSERT INTO public.landcover_class_weights (class_id, class_label, green_weight, is_urban) VALUES
    ...
    -- Canada land-cover-style classes (verify against your exact legend)
...
-- 1) Compute green_score + solitude_score from land cover and road density.
...
-- 2) Compute elevation_score from DEM (stddev normalized by 100m).
...
-- 3) Recompute composite scenic score using component scores.
...
SET scenic_score = LEAST(
    ...
    scoring_version = '2.2-raster-data-quality-upgrade';
...
SELECT
    ...
    ROUND(MIN(green_score)::numeric, 6) AS min_green_score,
    ROUND(MAX(green_score)::numeric, 6) AS max_green_score
FROM scenic_score_tiles;

SELECT
    ROUND(AVG(solitude_score)::numeric, 6) AS avg_solitude_score,
    ROUND(STDDEV_POP(solitude_score)::numeric, 6) AS stddev_solitude_score,
    ROUND(MIN(solitude_score)::numeric, 6) AS min_solitude_score,
    ROUND(MAX(solitude_score)::numeric, 6) AS max_solitude_score
FROM scenic_score_tiles;
...
SELECT
    ...
    ROUND(MIN(elevation_score)::numeric, 6) AS min_elevation_score,
    ROUND(MAX(elevation_score)::numeric, 6) AS max_elevation_score
FROM scenic_score_tiles;

SELECT
    ROUND(AVG(scenic_score)::numeric, 6) AS avg_scenic_score,
    ROUND(STDDEV_POP(scenic_score)::numeric, 6) AS stddev_scenic_score,
    ROUND(MIN(scenic_score)::numeric, 6) AS min_scenic_score,
    ROUND(MAX(scenic_score)::numeric, 6) AS max_scenic_score
FROM scenic_score_tiles;
